...
- Skill topology now includes exactly 26 skills across 10 groups.
...
- Wave 5 hook/runtime Slice 1 ships fail-closed command-hook enforcement over existing `ControlRodMode` and `ForemansWalk` truth with no HTTP hooks and no LLM involvement in hook decisions.
- Slice 2 extends that spine with compaction-safe preservation and bounded `SessionStart` re-injection (`startup`, `compact`, `resume`) using `PreCompact` + local runtime state only; `PostCompact` is not used in this slice.
...
- Dispatch Board supports `/walk`, `/phantoms`, `/change-order`, and `/control-rods`.
- Ticket System supports `/receipt`, `/walk`, `/phantoms`, and `/change-order`.
- Daily Log supports `/toolbox-talk`, `/receipt`, `/as-built`, and `/walk` and may render safety or hazard sections only when that truth exists in the canonical route view.
- Repair Order supports `/receipt` and `/as-built` and stays bounded to request/findings/performed-work or unresolved-exception framing.
- Kitchen Ticket supports `/walk`, `/phantoms`, and `/change-order` and stays item-first, terse, and timer-free.
- Farm Ledger supports `/toolbox-talk`, `/receipt`, `/as-built`, `/walk`, and `/change-order` and stays a chronological ledger / carry-forward record without fake farm-management fields.
- Safety / LOTO Log supports `/permit` and `/lockout` and stays an authorization / control record without fake industrial paperwork fields.
...
- Wave 5 skins tranche 4 rendering layer is now implemented at:
  - `docs/specs/SKIN_FRAMEWORK.md`
  - `src/SkinFramework.js`
  - `tests/golden/SkinFramework.golden.test.js`
- Wave 5 hook/runtime Slice 2 enforcement spine is now implemented at:
...
- tranche 4 skins rendering baseline for Whiteboard, Punch List, Inspection Report, Work Order, Dispatch Board, Ticket System, Daily Log, Repair Order, Kitchen Ticket, Farm Ledger, and Safety / LOTO Log over the approved route matrix with raw fallback for unsupported combinations
...
- Post-closeout structural plugin conversion now ships `.claude-plugin/plugin.json`, `hooks/hooks.json`, `hooks/run-governance-hook.js`, and plugin-discoverable `skills/<name>/SKILL.md`.
- This conversion does not add new governance engines or widen existing runtime contracts.
- Project `.claude/settings.json` remains the persistent deny-layer and standalone compatibility surface.
...
- `package.json` remains absent, so marketplace/package/install claims remain explicitly unverified.
- `docs/PLUGIN_CONVERSION_PROOF.md` is the canonical proof and local smoke-runbook surface for this conversion.
