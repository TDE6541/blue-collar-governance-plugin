...
- Change `docs/specs/HOOK_RUNTIME_ENFORCEMENT_SPINE.md` for hook runtime enforcement spine truth (Slice 2 core, Wave 6A Slice 3/Blocks B-D, Wave 6B Block A, Phase 1/2 lifecycle expansion, and Phase 3 Blocks A/B), including fail-closed command-hook scope, compaction survival/startup re-injection, Control Rod alignment, Stop/Walk closeout gate, persistent deny-layer truth, enforcement matrix (24 handled official lifecycle events), live chain population, permit gate, InstructionsLoaded observability, bounded `Elicitation`/`ElicitationResult` observability surfaces, `TaskCreated`/`TaskCompleted`/`TeammateIdle` behavior, and pending `WorktreeCreate`/`WorktreeRemove` boundary truth.
...
- Change `docs/PHASE3_LIFECYCLE_EXPANSION_CLOSEOUT.md` for durable current Phase 3 lifecycle expansion closeout truth, 24-event count reconciliation, historical integrity continuity notes, and finish-lane merge/push status.
- Change `docs/PHASE3_REMAINING_LIFECYCLE_SEAMS_CLOSEOUT.md` for the historical structural-lane record (A/B shipped, C held) without rewriting point-in-time truth.
- Change `docs/PHASE2_LIFECYCLE_EXPANSION_CLOSEOUT.md` for durable historical Phase 2 lifecycle expansion closeout truth, bounded MCP observability proof, and 19-to-21 count continuity notes.
...
## Wave 7 Truth Lock And Closure

- Change `docs/specs/WAVE7_TRUTH_LOCK.md` for the Wave 7 umbrella truth: C1 `/walk` closed, C2 `/fire-break` closed via a persisted hook-derived governance-health snapshot that is route-compatible for `/fire-break` (not canonical Open Items Board persistence), C3 plugin-native foreign-repo deny delivery closed through a plugin-owned, operator-invoked apply path (deterministic, reviewable, not runtime auto-injection, and not a universal compatibility claim), Work Order as the only intake pilot for Wave 7, current shipped truth that the existing Work Order skin/render surface still exists unchanged, the Work Order intake/scaffold/posture surfaces are shipped, the chain stops at reviewed artifacts only, Blocks D/E/F/G are shipped, and the parked/out-of-scope list (package/install, marketplace, Agent governance, multi-agent governance, trust-transfer/certificate work, second intake skin, and future-gated Anthropic work).
- Change `docs/specs/WORK_ORDER_INTAKE.md` for the Wave 7B Block D intake contract: required fields, HOLD reasons, canonical follow-up questions, and the intake-object-only boundary.
- Change `docs/specs/WORK_ORDER_SCAFFOLD.md` for the Wave 7B Block E scaffold contract: goal, scope, anti-goals, acceptance criteria, do-not-ship conditions, HOLD propagation, and the scaffold-object-only boundary.
- Change `docs/specs/WORK_ORDER_POSTURE.md` for the Wave 7B Block F posture contract: conservative recommendation lock, visible domain posture map, evidence mapping, and the no-hook-runtime-integration boundary.
- Change `docs/WAVE7_CLOSEOUT.md` for the Wave 7 closeout evidence map, commit sequence, remaining HOLDs, contract boundaries, and signoff state.
...
- Change `docs/specs/PACKET5_WALK_COMPOSITION_TRUTH_LOCK.md` and `docs/specs/WALK_CONFIDENCE_SIDECAR.md` for Packet 5 `/walk` confidence sidecar composition boundaries (render-side-only composition, supported sidecar section ids, and informational-only posture).
- Change `docs/specs/SESSION_LIFECYCLE_SKILLS.md` and `skills/walk/SKILL.md` for Packet 5 `/walk` sidecar composition contract wording, including persisted-walk-only wrapper/runtime posture (no auto-supplied sidecar input).
...
- Change `src/HookRuntime.js` for hook runtime enforcement behavior across all 24 handled official lifecycle events, including Wave 5 Slice 1/2 core paths, Wave 6A event handlers/chain writes/permit gate, Wave 6B InstructionsLoaded observability, Phase 1/2 lifecycle expansion surfaces, Phase 3 `TaskCreated`/`TaskCompleted`/`TeammateIdle` handling, observe-only `Elicitation`/`ElicitationResult` handling, and pending `WorktreeCreate`/`WorktreeRemove` boundary truth.
...
- Add `tests/live/wave5.hook-runtime.live.test.js` for hook runtime live integration proof across shipped enforcement behavior.
