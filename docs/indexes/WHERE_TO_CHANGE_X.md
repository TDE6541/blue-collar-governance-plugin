...
- Change `docs/specs/WAVE7_TRUTH_LOCK.md` for the Wave 7 umbrella truth: C1 `/walk` closed, C2 `/fire-break` closed via a persisted hook-derived governance-health snapshot that is route-compatible for `/fire-break` (not canonical Open Items Board persistence), C3 plugin-native foreign-repo deny delivery closed through a plugin-owned, operator-invoked apply path (deterministic, reviewable, not runtime auto-injection, and not a universal compatibility claim), Work Order as the only intake pilot for Wave 7, current shipped truth that the existing Work Order skin/render surface still exists unchanged, the Work Order intake pilot surface is shipped, the Work Order scaffold generation surface is shipped, Block E stops at the scaffold object only, Blocks F/G remain open, and the parked/out-of-scope list (package/install, marketplace, Agent governance, multi-agent governance, trust-transfer/certificate work, second intake skin, and future-gated Anthropic work).
...
- Change `README.md`, `CLAUDE.md`, `REPO_INDEX.md`, and `docs/INDEX.md` when the Wave 7 front-door sync for C1/C2/C3 seam status, Work Order-only intake wording, Block D/E boundary wording, open Blocks F/G, the parked list, or the canonical-engine-vs-hook-snapshot boundary changes.
